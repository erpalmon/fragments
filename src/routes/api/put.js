...
module.exports.updateFragment = async (req, res) => {
  ...
  try {
    ...
    if (fragment.type !== type) {
      logger.warn({ currentType: fragment.type, newType: type }, 'Fragment type cannot be changed');
      return res
        .status(400)
    ...
    logger.info({ id, type, size: req.body.length }, 'Fragment updated successfully');
